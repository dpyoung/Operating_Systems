...
int main(int ac, char *av[])
{
	if ( ac == 1 )  /* NOTE: Compare this main() to more01.c from ch1 */
		...
		while ( --ac )
		{
			if ( (do_recursion != 1) && (av[1][0] == '-') &&
				(av[1][1] == 'R' || av[1][1] == 'r'))
			{
				do_recursion = 1;
				*++av;  /* False positive Warning */
				/* set global flag to do recursion */
			}
			else
			{
				printf("%s:\n", *++av );
		...
		{			  /* DT_DIR not defined in ISOC99 Standard */
			...
			if (do_recursion == 1)
			{
				...
				/*I realized I could not keep this in an orderly print-out
					without using a queue (BLEGH! Data Structure!). So,
...
{ 
	...
	{	/*It can always stat! It wouldn't have a filename
			otherwise! We check for an error before
			even passing this a filename. */
